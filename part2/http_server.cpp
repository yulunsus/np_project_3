...
void reaper(int sig);
void server_function(int sockfd, string target_filename);
void write(int sockfd, char* msg);
...
int main(int argc, char* argv[])
{
	...
	while(true)
	{
		client_sockfd = accept(sockfd, (struct sockaddr *) &cli_addr, (socklen_t*) &c);
		if(client_sockfd < 0){
		...
		if(childpid < 0)
    		...
    		close(sockfd);
			server_function(client_sockfd, target_filename);  
    		close(client_sockfd);
...
void server_function(int sockfd, string target_filename)
{
	...
	while((Connection_pos = buffer_string.find("Connection")) == -1){
	...
	char* request_cstr = new[BUFFER_SIZE];
	...
	if(strstr(target_filename,".cgi") != NULL){

		dup2(sockfd, 1);
		dup2(sockfd, 2);
		execl(target_filename.c_str(), target.filename.c_str(), NULL);
	}
	else{
